...
public:
    vector<int> maxKDistinct(vector<int>& nums, int k) {
        // store input midway
        vector<int> originalInput = nums;

        // remove duplicates
        unordered_set<int> unique(nums.begin(), nums.end());
        vector<int> distinct(unique.begin(), unique.end());

        // sort in descending order
        sort(distinct.begin(), distinct.end(), greater<int>());

        // pick at most k elements
        vector<int> result;
        for (int i = 0; i < min(k, (int)distinct.size()); i++) {
...
// TC=O(nlogn)  (because of sorting), SC=O(n)  for distinct elements and result