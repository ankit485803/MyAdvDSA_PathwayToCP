...
class Solution {
public:
    bool repeatedSubstringPattern(string s) {  //TC=O(n) = SC due to the creation of the doubled string
        int n = s.length();
        // Create a new string by concatenating s with itself
        string doubled = s + s;
        
        // Find the first occurrence of s in doubled, starting from index 1 and ending before the last character
        // We are checking if `s` is a substring in the new string (excluding the first and last characters)
...
qno 2154  https://leetcode.com/problems/keep-multiplying-found-values-by-two/description/  
...
class Solution {
public:
    int findFinalValue(vector<int>& nums, int original) { //TC=O(n) SC=O(1)
        
        // Continue the process as long as 'original' is found in 'nums'
        while(find(nums.begin(), nums.end(), original)  !=  nums.end()) {
            original = 2*original;  // Multiply 'original' by 2 if it's found
