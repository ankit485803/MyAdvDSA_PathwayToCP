...
qno 3532  https://leetcode.com/problems/path-existence-queries-in-a-graph-i/description/?envType=daily-question&envId=2026-07-09
    ...
    vector<bool> pathExistenceQueries(int n, vector<int>& nums, int maxDiff, vector<vector<int>>& queries) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            ...
            if (abs(num1 - num2) <= maxDiff) {
                unite(i, i + 1);                 // within gap -> merge the two nodes
            }
        }
        ...
        for (const auto& q : queries) {
            int x = q[0];
            int y = q[1];
            ans.push_back(find(x) == find(y));   // same root -> path exists
        }
        return ans;
    }
